...
- **component**: One of `runtime_core`, `editor`, `graphics`, `module`, `job_system`, `serialization`, `scripting`, `ui`, `animation`, `physics`, `audio`, `platform_layer`, `networking`, `build_system`, `testing`, `documentation`, `tooling`
...
- **root_cause**: One of `wrong_api`, `logic_error`, `memory_leak`, `memory_corruption`, `null_reference`, `stack_overflow`, `race_condition`, `thread_violation`, `async_timing`, `serialization_error`, `config_error`, `platform_specific`, `missing_validation`, `missing_permission`, `missing_workflow_step`, `inadequate_documentation`, `missing_tooling`, `incomplete_setup`
...
- **unity_version**: Unity version (e.g. `2022.3.45f1`)
...
9. `unity_version`, if present, must match Unity version format (e.g. `2022.3.45f1`) and only applies to bug-track docs.
